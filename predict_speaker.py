# pip install joblib librosa numpy sounddevice soundfile
import joblib
import librosa
import numpy as np
...
def voicesamples():
    for audio_file in audios:
        ...
        if file_path.endswith('.flac') or file_path.endswith('.wav'):
            extracted_features = extract_features(file_path)
            extracted_features = extracted_features.reshape(1, -1)
            prediction = model.predict(extracted_features)
            expected.append(audio_file)
            predicted.append(prediction[0])
            if prediction[0] not in count:
                count[prediction[0]] = 1
            else:
                count[prediction[0]] += 1
...
def record():
    ...
    new_features = extract_features("predict.wav")
    print(new_features)
